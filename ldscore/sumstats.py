...
def _read_ref_ld(args, log):
    ...
    ref_ld = _read_chr_split_files(args.ref_ld_chr, args.ref_ld, log, "reference panel LD Score", ps.ldscore_fromlist)
    ...
    try:
        if args.ref_ld is not None:
            overlap_matrix, M_tot = _read_chr_split_files(
                args.ref_ld_chr, args.ref_ld, log, "annot matrix", ps.annot, frqfile=args.frqfile
            )
        elif args.ref_ld_chr is not None:
            overlap_matrix, M_tot = _read_chr_split_files(
                args.ref_ld_chr, args.ref_ld, log, "annot matrix", ps.annot, frqfile=args.frqfile_chr
            )
    except Exception:
