...
def test_check_variance():
    ld = pd.DataFrame({"SNP": ["a", "b", "c"], "LD1": np.ones(3).astype(float), "LD2": np.arange(3).astype(float)})
    ld = ld[["SNP", "LD1", "LD2"]]
...
class Test_RG_Statistical:
    ...
    def test_rg_se(self):
        assert_allclose(
            np.nanmean(list(map(t("rg_se"), self.rg))), np.nanstd(list(map(t("rg_ratio"), self.rg))), atol=0.02
        )
    ...
    def test_gencov_tot(self):
        assert_allclose(np.nanmean(list(map(t("tot"), list(map(t("gencov"), self.rg))))), 0, atol=0.02)

    def test_gencov_tot_noint(self):
        assert_allclose(np.nanmean(list(map(t("tot"), list(map(t("gencov"), self.rg_noint))))), 0, atol=0.02)
    ...
    def test_gencov_cat(self):
        assert_allclose(np.nanmean(list(map(t("cat"), list(map(t("gencov"), self.rg))))), [0, 0], atol=0.02)

    def test_gencov_cat_noint(self):
        assert_allclose(np.nanmean(list(map(t("cat"), list(map(t("gencov"), self.rg_noint))))), [0, 0], atol=0.02)
    ...
    def test_gencov_int(self):
        assert_allclose(np.nanmean(list(map(t("intercept"), list(map(t("gencov"), self.rg))))), 0, atol=0.1)
    ...
    def test_hsq_int(self):
        assert_allclose(np.nanmean(list(map(t("intercept"), list(map(t("hsq2"), self.rg))))), 1, atol=0.1)
...
class Test_H2_Statistical(unittest.TestCase):
    ...
    def test_tot_se(self):
        assert_allclose(np.nanmean(list(map(t("tot_se"), self.h2))), np.nanstd(list(map(t("tot"), self.h2))), atol=0.05)

    def test_tot_se_noint(self):
        assert_allclose(
            np.nanmean(list(map(t("tot_se"), self.h2_noint))), np.nanstd(list(map(t("tot"), self.h2_noint))), atol=0.05
        )
    ...
    def test_coef_se(self):
        for h in [self.h2, self.h2_noint]:
            assert_array_almost_equal(
                np.nanmean(list(map(t("coef_se"), h)), axis=0), np.nanstd(list(map(t("coef"), h)), axis=0)
            )
    ...
    def test_int_se(self):
        assert_allclose(
            np.nanstd(list(map(t("intercept"), self.h2))), np.nanmean(list(map(t("intercept_se"), self.h2))), atol=0.1
        )
